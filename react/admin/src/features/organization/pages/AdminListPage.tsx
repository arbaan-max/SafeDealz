...
export function AdminListPage() {
  ...
  return (
    <ResourcePage eyebrow="A23" title="Admins" lede="Super Admin only. Each Admin sees only the stores you assign." action={<Link className="login-button" to="/admins/new">Create admin</Link>}>
      ...
      <DataTable
        ...
        rows={admins.map((admin) => [
          ...
          <Link key={`${admin.id}-edit`} to={`/admins/${admin.id}`}>Manage</Link>,
        ])}
      />
    </ResourcePage>
  );
