...
| P06–P09 | Validate allowlisted fields and ownership, prevent NoSQL/operator injection; private media with type/size/content checks and short-lived authorized access; camera permissions; QR signature/device binding/expiry/replay checks; no KYC exposure in QR. |
| P10–P11 | Atomic balances, integer currency, server-calculated values, idempotency and concurrency tests; verified provider webhook signature on raw body, replay protection and provider-event uniqueness; never trust client payment success. |
