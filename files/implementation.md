...
Client theme amendment on 2026-09-13: Sky Blue is now the confirmed primary theme. The interactive prototype defaults to Sky Blue, all three Flutter foundations use the shared sky action color, native splash colors match, Obsidian uses the same accent, and the design/architecture/phase records define Purple only as a restrained secondary or prototype comparison accent.
...
| Sky-blue theme | Pending current verification: Flutter analysis/tests plus prototype token/default checks |
