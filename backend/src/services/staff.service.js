import { createStaffAccount, listStaff, updateStaffAccount } from './organization.service.js';
...
export const listManagers = (actor) => listStaff(actor, { role: 'store_manager', kind: 'store_manager' });
...
export const createVendor = async (actor, body) => {
  if (body.existingEmail) throw new ApiError(400, 'VALIDATION_ERROR', 'Use the dedicated link action to attach an existing vendor.');
  const { account, assignedBranchIds } = await createStaffAccount(actor, {
    role: 'vendor', kind: 'vendor', body, singleBranch: false,
  });
  const wallet = await createWallet({ vendorAccountId: account.id, availablePaise: 0, reservedPaise: 0 });
