...
import 'package:safedealz_vendor/data/api/models/refresh_request.dart';
...
class AuthService {
  ...
  Future<void> login(String email, String password) async {
    try {
      ...
      await _tokens.save(r.data.accessToken, refresh);
  ...
  Future<String?> _refresh() async {
    ...
    try {
      final r = await _client.refresh(
        body: RefreshRequest(
          clientType: ClientType.mobile,
          refreshToken: token,
        ),
      );
      final next = r.data.refreshToken;
      if (next == null) {
        throw const AuthFailure('SESSION_INVALID', 'Session expired.');
      }
      await _tokens.save(r.data.accessToken, next);
  ...
  Future<Never> _map(DioException e) async {
