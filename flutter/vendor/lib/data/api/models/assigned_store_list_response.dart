// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, unused_import, invalid_annotation_target, unnecessary_import

import 'package:freezed_annotation/freezed_annotation.dart';

import 'assigned_store.dart';

part 'assigned_store_list_response.freezed.dart';
part 'assigned_store_list_response.g.dart';

@Freezed()
abstract class AssignedStoreListResponse with _$AssignedStoreListResponse {
  const factory AssignedStoreListResponse({
    required bool success,
    required List<AssignedStore> data,
  }) = _AssignedStoreListResponse;
  
  factory AssignedStoreListResponse.fromJson(Map<String, Object?> json) => _$AssignedStoreListResponseFromJson(json);
}
